...
- Phase 1D (part): stack creator background can be Colour, Gradient or Transparent. Gradient uses the shared `GradientColourEditor` and `useGradientCanvas` (also now used by the gradient generator), with chaos, grain and New arrangement. URL keys `bg=gradient&gColours=&gChaos=&gGrain=&gSeed=`. PNG/JPEG exports draw the stack over the full-size gradient; SVG embeds the gradient as a PNG data URI.
...
- Screenshots reviewed for every page at 1440px and the stack creator at 390px.
- Second pass after gradient backgrounds: 19 tests, type-check, lint and build pass. Headless: switching to Gradient updates the URL; PNG, JPEG and SVG exports are 1920×1080 and the JPEG matches the preview; SVG with gradient is 4.8 MB; Undo returns to the colour background; a 1179×2556 frame keeps its aspect ratio (0.461); gradient generator export still works after the refactor; no console errors.
...
- Mobile layout of the gradient generator.
...
4. Phase 1D: private preview deployment (needs hosting and visibility decisions).
